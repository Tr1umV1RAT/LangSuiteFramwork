...
function Ensure-HealthyVenv {
    ...
    if (-not (Test-Path $PythonExe)) {
        $recreateReason = 'virtual environment python.exe is missing'
    } else {
        try {
            & $PythonExe -m pip --version *> $null
        } catch {
            $recreateReason = 'virtual environment pip is unhealthy'
        }
    }
    if ($recreateReason) {
        Write-Step "Recreating incomplete virtual environment: $recreateReason" 'Yellow'
        if ($DryRun) {
            Write-Step "[dry-run] Would remove $VenvDir" 'Yellow'
            Write-Step "[dry-run] Would create Python virtual environment in $VenvDir" 'Yellow'
        } else {
