...
def temporary_local_url_policy_lift():
    ...
    try:
        if CHROMIUM_POLICY_DIR.exists():
            backup_dir = CHROMIUM_POLICY_DIR.with_name('managed.bak_langsuite_v35')
...
def run():
    ...
    try:
        wait_for_http(f'http://127.0.0.1:{port}/?e2e=v35')
        with temporary_local_url_policy_lift():
            with sync_playwright() as pw:
                with tempfile.TemporaryDirectory(prefix='langsuite-v35-chromium-') as profile_dir:
                    ...
                    bridge_info = page.evaluate("""async () => {
                        ...
                        const agent = payload.find((item) => item.kind === 'agent' && item.id === 'minimal_agent');
                        return agent ? {
                    ...
                    expect(bridge_info is not None, 'expected minimal_agent in LangGraph advanced library')
                    ...
                    expect(bridge_info['bridgeTargetMode'] == 'langgraph', f'bridge target mismatch: {bridge_info}')
                    ...
                    expect(compiled['ok'] is True, f'compile failed for executable bridge: {compiled}')
                    expect('application/zip' in compiled['contentType'], f'expected zip compile response: {compiled}')

                    opened = page.evaluate("""async () => {
                        const store = window.__LANGSUITE_STORE__;
                        const wrapper = [...store.getState().nodes].reverse().find((node) => node.data?.params?.artifact_ref_id === 'minimal_agent');
