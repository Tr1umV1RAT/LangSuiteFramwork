...
def run():
    ...
    with tempfile.TemporaryDirectory(prefix='langsuite-v25-e2e-') as tmpdir:
        proc = start_backend(Path(tmpdir) / 'e2e.db', free_port())
        port = int(proc.args[-1])
        try:
            wait_for_http(f'http://127.0.0.1:{port}/?e2e=1')
            unique = f'LangSuite v25 Smoke {int(time.time())}'
            with sync_playwright() as pw:
                ...
                package_payload = page.evaluate("""() => {
                    return JSON.parse(window.__LANGSUITE_STORE__.getState().exportProjectPackage());
                }""")
                ...
                expect('Restored the editable workspace only' in report_ui.inner_text(), 'success import message not tightened')
                ...
                expect('Restored the editable workspace partially' in page.locator('[data-testid="package-import-report"]').inner_text(), 'partial package warning summary not rendered')
