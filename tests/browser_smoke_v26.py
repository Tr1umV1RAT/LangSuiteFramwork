...
def temporary_local_url_policy_lift():
    ...
    try:
        if CHROMIUM_POLICY_DIR.exists():
            backup_dir = CHROMIUM_POLICY_DIR.with_name('managed.bak_langsuite_v26')
...
def run():
    ...
    with tempfile.TemporaryDirectory(prefix='langsuite-v26-e2e-') as tmpdir:
            ...
            with temporary_local_url_policy_lift():
                with sync_playwright() as pw:
                    ...
                    static_node_id = page.evaluate("""() => {
                        ...
                        store.getState().updateNodeParam(node.id, 'text', 'Hello from v26 smoke');
                        return node.id;
                    }""")
                    ...
                    expect(node_metrics['maxWidth'] in ('300px', '308px'), f'node card max-width did not reflect the tighter CSS: {node_metrics}')
