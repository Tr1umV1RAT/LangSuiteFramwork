...
class ObsidianEncounterPatch(BaseModel):
    ...
    successAtCost: Optional[str] = None
    falloutOnFail: Optional[str] = None
    mergePolicy: str = "replace"

    @field_validator("encounterId")
    @classmethod
    def validate_encounter_id(cls, v: str) -> str:
        return _require_safe_identifier(v, label="obsidian.encounterPatch.encounterId")
    ...
    def validate_status(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return None
        if v not in {"seeded", "active", "resolved"}:
            raise ValueError("obsidian.encounterPatch.status must be seeded, active, or resolved")
        return v

    @field_validator("pressure")
    @classmethod
    def validate_pressure(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return None
        if v not in {"low", "medium", "high"}:
            raise ValueError("obsidian.encounterPatch.pressure must be low, medium, or high")
        return v
    ...
    def validate_merge_policy(cls, v: str) -> str:
        if v not in {"error", "preserve", "replace"}:
            raise ValueError("obsidian.encounterPatch.mergePolicy must be error, preserve, or replace")
        return v


class ObsidianLocationPatch(BaseModel):
    locationId: str
    status: Optional[str] = None
    ...
    @classmethod
    def validate_location_id(cls, v: str) -> str:
        return _require_safe_identifier(v, label="obsidian.locationPatch.locationId")
    ...
    def validate_status(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return None
        if v not in {"seeded", "active", "resolved"}:
            raise ValueError("obsidian.locationPatch.status must be seeded, active, or resolved")
        return v
    ...
    def validate_merge_policy(cls, v: str) -> str:
        if v not in {"error", "preserve", "replace"}:
            raise ValueError("obsidian.locationPatch.mergePolicy must be error, preserve, or replace")
        return v
